...
local SPREADSHEET_MINOR = tonumber(("$Rev: 79 $"):match("(%d+)")) or 10000;
...
function LibSpreadsheet:CloseBook(ignore)
    for k, sheet in pairs(self.sheets) do
        LibSpreadsheet:UnloadSheet(sheet, self, ignore);
    end
	if (not ignore) then LibSpreadsheet:DeleteFrame(self.frame); end
	self.frame = nil;
	if ( self.frames ) then
	...
	self.parent = nil;
	self.hasBookUI = false;
    LibSpreadsheet.books[self] = nil;
end

function LibSpreadsheet:UnloadSheet(sheet, book, ignore)
    ...
    for key, column in pairs(sheet.columns) do
        if ( column.frame ) then
            if (not ignore) then LibSpreadsheet:DeleteFrame(column.frame); end
            column.frame = nil;
            column.h = nil;
            column.height = nil;
            column.w = nil;
    ...
    if ( sheet.container and not ignore ) then
    ...
    for key, row in pairs(sheet.rows) do
        if ( row.frame ) then
            if (not ignore) then LibSpreadsheet:DeleteFrame(row.frame); end
            row.frame = nil;
            row.u = true;
            if ( row.axis ) then
                if ( not ignore) then LibSpreadsheet:DeleteFrame(row.axis); end
                row.axis = nil;
            end
        end
        for key, cell in pairs(row.cells) do
            if ( cell.frame ) then
               if ( not ignore) then LibSpreadsheet:DeleteFrame(cell.frame); end
            end
...
function LibSpreadsheet:DeleteFrame(frame)
	if ( frame and frame.SetParent ) then
		...
		for _, child in ipairs({frame:GetChildren()}) do
			...
			if ( child.SetTexture ) then child:SetTexture(nil); end
		end
		if ( frame.xt ~= "slider" ) then
...
function LibSpreadsheet:BuildFramework()
	...
	table.insert(self.frames, self.frame);
	...
	table.insert(self.frames, self.scrollFrame);
	...
	table.insert(self.frames, self.verticalScrollbar);
    local s2 = sqrt(2);
	...
	table.insert(self.frames, self.horizontalScrollbar);
    local tex, button;
    ...
    self.editbox = LibSpreadsheet:CreateFrame("EditBox", string.format("Spreadsheet_%x", math.floor(math.random(time()))), self.frame, "InputBoxTemplate");
    ...
    self.editbox:Hide();
    self.hasBookUI = true;
	table.insert(self.frames, self.sheetSelector);
	table.insert(self.frames, self.prevSheet);
	table.insert(self.frames, self.nextSheet);
	table.insert(self.frames, self.editbox);
end
...
--        self.horizontalScrollbar:Show();
    ...
    local now = GetTime();
    if ( now - self.lastUpdate > 0.15 ) then -- We really don't need to update more than 5 times per second, do we? ;<
...
function LibSpreadsheet:Render_Update(colStart, rowStart, caller)
	...
	self.verticalScrollbar:SetValue(rowStart);
...
--	print("Render_Real:", time(), colStart, rowStart, colEnd, rowEnd)
    local sheet = self.selectedSheet;
